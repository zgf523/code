...
drivermemory=10g
executormemory=20g
SPARK_ARG="--class com.yiwugou.iProbe.dao.user.expe.BaseInfoLogMain5 --master spark://node11:7077 --conf spark.default.parallelism=$parallelism --total-executor-cores $core --driver-memory $drivermemory --executor-memory $executormemory"

if [ $# -ne 2 ]
then 
	echo "error input ! "
	echo "please input arg : start date and end date"
	echo "example ./xxx.sh 1012a 100"
	exit
fi
...
File=$dirstr/$filepre$filename    
    ...
    echo $File
#   submit job
    `($SPARK_HOME/bin/spark-submit $SPARK_ARG $JOB_NAME $File $2 &)` >>  ~/script/timer/logs/kpijob.log
